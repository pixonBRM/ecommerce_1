...
uploadImages.forEach((fileupload, index) => {                              //use forEach method to loop for each file uploaded
    fileupload.addEventListener('change', () => {                       // change event to file upload or upload input
        const file = fileupload.files[0];                              // access the uploaded file
        let imageUrl;                                                 // define the image url variable to store the link 

        if(file.type.includes('image')){                             //check if the file contains .type or not
            // means user uploaded an image 
            fetch('/s3url').then(res => res.json())
            .then(url => {
                fetch(url, {                                          // after the url is acquire another fetch method is used to PUT a request to the url
                    method: 'PUT',
                    headers: new Headers({'Content-Type': 'multipart/form-data'}),
                    body: file                                        // send file as the body
                }).then(res => {                                      // use .then to catch the response
...
const validateForm = () => {
    if(productName.value.length){
        ...
        return shortAlert ('short description must be between 10 to 100 letters long');
...
const productData = () => {
    ...
    return data = {            //this is the data that is stored in the database
...
addProductBtn.addEventListener('click', () => {
    ...
    if(validateForm()){   // validateForm return true or false while doing validation
        ...
        if(productId){     // to avoid generating new product id when product id is not equal to null
            data.id = productId;  
        }
