...
window.onload = () => {
  if (sessionStorage.user) {
    //check if any user exists in session or not
...
submitBtn.addEventListener("click", () => {
  ...
  if (name != null) {
    // check if name is null or not if not that means we are on sign up page
    if (name.value.length < 3) {
      // if -to validate the form name.value - to validate and call field value ang length -gives the number of letters in the string
      showAlert("name must be 3 letters long");
    } else if (!email.value.length) {
      // ! shows the boolean type (changing true into false and false into true)
      ...
      // since we are using 'text' in the number field and Number is a js which the parameter is a number or not, just parse the text field value to check if the user entered a numerical value
      ...
      sendData("/signup", {
        // call the function (sendData), parse signup as the first argument and object as second argument
        name: username.value,
        email: email.value,
    ...
    if (!email.value.length || !password.value.length) {
      ...
      loader.style.display = "block"; // setting the loader to display in a block method
      sendData("/logging", {
        // call the function (sendData) to send the data, parse signup as the first argument and object as second argument
        email: email.value,
        password: password.value,
      });
    }
...
const processData = (data) => {
  ...
  if (data.alert) {
    // to check if the data as alert key or not
...
//alert function
const showAlert = (msg) => {
  //creating the function with its argument
  ...
  setTimeout(() => {
    // is set to remove the show class after 3 sec
